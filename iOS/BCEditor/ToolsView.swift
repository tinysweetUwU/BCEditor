...
struct ToolDetail: View {
    ...
    var body: some View { Form {
        ...
        if tool.title == "Cats & Skills" { Section("Direct edit") { TextField("Cat ID", text: $catID).keyboardType(.numberPad); TextField("Base level", text: $catLevel).keyboardType(.numberPad); Button("Apply cat level", systemImage: "pencil") { if let id = Int(catID), let level = Int(catLevel) { store.applyNativeAction("set_cat_level", value: (id << 16) | level) } }.disabled(!store.hasSave); action("Unlock all cats", "unlock_all_cats", "lock.open.fill"); action("Max cats and forms", "max_cats", "star.fill"); action("Max talents", "max_talents", "sparkles"); action("Max special skills", "max_special_skills", "bolt.fill"); action("Unlock Cat Guide", "unlock_cat_guide", "book.fill"); action("Fill Cat Storage", "fill_cat_storage", "tray.full.fill"); action("Clear Cat Storage", "clear_cat_storage", "trash") } }
        if tool.title == "Levels" { Section("Direct edit") { action("Unlock Aku Realm", "unlock_aku_realm", "lock.open.fill"); action("Clear available stages", "clear_all_maps", "map.fill"); action("Allow Filibuster replay", "allow_filibuster", "repeat.circle") } }
        if tool.title == "Gamatoto" { Section("Direct edit") { action("Max Gamatoto XP", "max_gamatoto", "hammer.fill"); action("Max Gamatoto materials", "max_gamatoto_materials", "shippingbox.fill"); action("Max Cat Shrine", "max_cat_shrine", "sparkles"); action("Max Ototo cannons", "max_ototo_cannons", "bolt.fill") } }
